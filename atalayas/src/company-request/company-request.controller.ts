import { Controller, Get, Post, Body, Patch, Param, Delete, UseInterceptors, UploadedFile, UseGuards, ParseUUIDPipe, BadRequestException } from '@nestjs/common';
...
export class CompanyRequestController {
  ...
  async create(@Body() createCompanyRequestDto: CreateCompanyRequestDto, @UploadedFile() file: Express.Multer.File) {
  ...
  @ApiBody({
    schema:{
      type: 'object',
      properties: {
        rejectReason: {type: 'string', example: 'La empresa no pertenece al polígono.'}
      }
    }
  })
