...
export class ServicesService {
  ...
  async create(createServiceDto: CreateServiceDto, requestUser: User) {
     if (requestUser.role === 'EMPLOYEE' || requestUser.role === 'PUBLIC') {
        ...
        const companyId = requestUser.role === 'GENERAL_ADMIN' && createServiceDto.companyId
          ? createServiceDto.companyId
          : requestUser.companyId;
    
          if(!companyId) {
            throw new ForbiddenException('No puedes asignar un servicio a una empresa sin especificar un ID de empresa válido');
          }
  ...
  async findOne(id: string, requestUser: User) {
    ...
    if (requestUser.role === 'GENERAL_ADMIN' && !service.isPublic && service.companyId !== requestUser.companyId) {
        throw new ForbiddenException('No tienes permisos para ver este servicio');
    }
      return service;
    }

  async update(id: string, updateServiceDto: UpdateServiceDto, requestUser: User) {
    if(requestUser.role === 'EMPLOYEE' || requestUser.role === 'PUBLIC') {
    ...
    if (requestUser.role === 'ADMIN' && service.companyId !== requestUser.companyId) {
      throw new ForbiddenException('No tienes permisos para actualizar servicios de otra empresa');
    }
  ...
  async remove(id: string, requestUser: User) {
    ...
    if (requestUser.role === 'ADMIN' && service.companyId !== requestUser.companyId) {
      throw new ForbiddenException('No tienes permisos para eliminar servicios de otra empresa');
    }
